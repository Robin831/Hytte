category: Added
- **Recipe detail page with portion scaling** - Open a recipe to read its ingredients and method, rate it, log a cook, and edit every field, tag, ingredient and step. A portion control rescales the displayed amounts (rendering 1.5 dl as "1 1/2 dl") without ever touching the stored recipe, so saving an edit from a scaled view still writes the base quantities. (Hytte-wevzx)
