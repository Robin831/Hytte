...
func Solve(board *SolverBoard, rackStr string, trie *Trie) *SolveResult {
	...
	// Extend each move to include any adjacent board tiles in the placement
	// direction. The Appel-Jacobson algorithm may generate a word via leftPart
	// that doesn't include existing tiles adjacent to the placed letters (e.g.,
	// placing "NÅ" next to an existing "U" should be validated as "UNÅ").
	for i := range allMoves {
		extendMoveInline(board, &allMoves[i])
	}
...
// extendMoveInline extends a rawMove to include any existing board tiles
// that are contiguous with the placed word in the placement direction.
// This ensures the full inline word is validated and scored, not just the
// portion generated by leftPart/extendRight.
func extendMoveInline(board *SolverBoard, m *rawMove) {
	dr, dc := 0, 1 // horizontal
	if m.dir == dirVertical {
		dr, dc = 1, 0
	}

	// Walk backward from start to find prefix tiles on the board
	var prefix []rune
	r, c := m.row-dr, m.col-dc
	for r >= 0 && c >= 0 && r < BoardSize && c < BoardSize && board.Cells[r][c].Filled {
		prefix = append(prefix, board.Cells[r][c].Letter)
		r -= dr
		c -= dc
	}

	// Walk forward from end to find suffix tiles on the board
	wordLen := len(m.word)
	var suffix []rune
	r = m.row + wordLen*dr
	c = m.col + wordLen*dc
	for r >= 0 && c >= 0 && r < BoardSize && c < BoardSize && board.Cells[r][c].Filled {
		suffix = append(suffix, board.Cells[r][c].Letter)
		r += dr
		c += dc
	}
	...
	// Reverse prefix (it was collected backwards)
	for i, j := 0, len(prefix)-1; i < j; i, j = i+1, j-1 {
		prefix[i], prefix[j] = prefix[j], prefix[i]
	}

	// Build extended word and isBlank arrays
	newWord := make([]rune, 0, len(prefix)+len(m.word)+len(suffix))
	newWord = append(newWord, prefix...)
	newWord = append(newWord, m.word...)
	newWord = append(newWord, suffix...)

	newIsBlank := make([]bool, len(newWord))
	// Prefix and suffix positions are existing board tiles (not blank from rack).
	// Copy the original isBlank values into the middle portion.
	copy(newIsBlank[len(prefix):], m.isBlank)

	m.word = newWord
	m.isBlank = newIsBlank
	m.row -= len(prefix) * dr
	m.col -= len(prefix) * dc
}
