...
func NewRouter(db *sql.DB) http.Handler {
	...
	training.OnRaceClassified = func(db *sql.DB, workoutID, userID int64, workoutDate string, distanceMeters float64) error {
	...
	// triggers an immediate evaluation for the workout's date. Nothing else
	// evaluates on a schedule — the nightly cron is gone — so this callback and
	// the per-day "Re-run evaluation" button are the only triggers. Same
	// cycle-breaking pattern as the race-matching callback above.
		...
		r.Group(func(r chi.Router) {
			...
			r.Group(func(r chi.Router) {
				...
				if os.Getenv("FEATURE_FORGE_DASHBOARD") == "1" {
					...
					r.Post("/forge/ext-prs/fix-comments", forge.FixCommentsExternalPRHandler(forgeDB))
					r.Post("/forge/ext-prs/fix-ci", forge.FixCIExternalPRHandler(forgeDB))
					r.Post("/forge/ext-prs/fix-conflicts", forge.FixConflictsExternalPRHandler(forgeDB))
					r.Post("/forge/ext-prs/bellows", forge.BellowsExternalPRHandler(forgeDB))
					r.Post("/forge/ext-prs/reset-counters", forge.ResetCountersExternalPRHandler(forgeDB))
			...
			r.Group(func(r chi.Router) {
				...
				r.Get("/budget/upcoming", budget.UpcomingHandler(db))
			...
			r.Group(func(r chi.Router) {
				...
				r.Get("/training/tags", training.TagsHandler(db))
				...
				r.Get("/training/workouts/{id}/context", training.GetWorkoutContextHandler(db))
				r.Put("/training/workouts/{id}/context", training.PutWorkoutContextHandler(db))
			...
			r.Group(func(r chi.Router) {
				...
				r.Patch("/stride/notes/{id}", stride.UpdateNoteHandler(db))
				...
				r.Post("/stride/plans/{planId}/chat/reset", stride.StrideChatResetHandler(db))
				r.Get("/stride/evaluations", stride.ListEvaluationsHandler(db))
				r.Get("/stride/evaluations/{id}/messages", stride.EvalMessagesListHandler(db))
				r.Post("/stride/evaluations/{id}/messages", stride.EvalMessagesSendHandler(db))
				...
				r.Get("/stride/macro/current", stride.GetCurrentMacroPlanHandler(db))
				r.Post("/stride/macro/generate", stride.GenerateMacroPlanHandler(db))
				r.Post("/stride/macro/extend", stride.ExtendMacroPlanHandler(db))
				r.Get("/stride/macro/{id}", stride.GetMacroPlanHandler(db))
			...
			r.Group(func(r chi.Router) {
				...
				r.Delete("/chat/conversations/{id}/messages/{messageID}", chat.TruncateHandler(db))
			})
			...
			r.Group(func(r chi.Router) {
				...
				r.Patch("/familychat/conversations/{id}/messages/{messageID}", familychat.EditMessageHandler(db))
				r.Delete("/familychat/conversations/{id}/messages/{messageID}", familychat.DeleteMessageHandler(db))
			...
			r.Group(func(r chi.Router) {
				...
				r.Get("/math/achievements", mathgame.AchievementsHandler(db))
			})
