...
export default function HomeworkSettings() {
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-6">
      ...
      {error && (
        <div role="alert" className="mb-4 px-4 py-2 bg-red-900/50 border border-red-800 rounded-lg text-red-300 text-sm">
          {error}
        </div>
      )}

      {success && (
        <div role="status" className="mb-4 px-4 py-2 bg-green-900/50 border border-green-800 rounded-lg text-green-300 text-sm">
          {success}
        </div>
      )}
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
            ...
            {AVAILABLE_SUBJECTS.map(subject => {
              ...
              >
                <input
                  ...
                  className="sr-only"
                  aria-label={t(`settings.subjectNames.${subject}`)}
                />
                <span aria-hidden="true" className={`w-4 h-4 rounded border flex items-center justify-center shrink-0 ${
                  checked
        ...
        <div>
          ...
          <input
            ...
            type="text"
            value={schoolName}
            onChange={e => { clearSuccess(); setSchoolName(e.target.value) }}
        ...
        <div>
          ...
          <textarea
            id="hw-topics"
            value={currentTopics}
            onChange={e => { clearSuccess(); setCurrentTopics(e.target.value) }}
            rows={4}
