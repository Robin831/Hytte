...
export default function WorkHoursWidget() {
  ...
  const [{ loading, summary, punched }, dispatch] = useReducer(reducer, {
    loading: true,
    summary: null,
    punched: false,
  })
  ...
  const target = 7.5 * 60 // 7h 30m standard Norwegian workday
  const remaining = Math.max(0, target - summary.net_minutes)
